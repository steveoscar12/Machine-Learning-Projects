...
% Last Modified by GUIDE v2.5 06-Aug-2018 15:56:29
...
[lol,FinalTable_size] = size(FinalTable);
[pol,TestFinalTable_size] = size(TestFinalTable);
array = zeros(lol,FinalTable_size);
for j = 1:1:FinalTable_size %column     
    for i = 1:1:lol %row
     value = FinalTable(i,j)- TestFinalTable(i);
     if value == 0
         array(i,j) = 1;
     elseif  value <= 0.2
         array(i,j) = FinalTable(i,j);
     elseif value>0.2
         array(i,j)=0;
     end
    end
...
ending = sum(~isnan(TArray(:,:)));
arrange = 1:pol;
first8  = sum(~isnan(TArray(1:8,:)));
first8 = 8-(8- first8);
second45 =sum(~isnan(TArray(9:53,:)));
second45 = 45 - (45 - second45);
NvsP = [first8;second45];
ending1 = [ending(1,:);NvsP(:,:)];
appended = [ending1(:,:);TArray(:,:)];

%sorting the order

[temp, order] = sort(appended(1,:));
appended = appended(:,order);
 
%Calculations of Value

    %count the number of NaN in a row
    %subtract Nan from 1st 8
    %subtract NaN from 2nd 53-8
...
% handles    structure with handles and user data (see GUIDATA)
[filename ,pathname] = uigetfile({'*.xls'},'File Selector');
imagesrc = strcat(pathname,filename);
dataset =xlsread(imagesrc,'Sheet1');
set(handles.uitable2,'data',dataset);
